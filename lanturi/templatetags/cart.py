...
@register.inclusion_tag(filename='lanturi/tags/cart.html', name='cart_link')
def get_cart_link(session):
    cart = session.get('cart', {})
    lant_ids = cart.keys()

    print('cart', cart)
    lanturi = Produse.objects.filter(id__in=lant_ids)
    print('cart.values()', list(cart.values()))

    total = sum(
        [
            float(lant.price) * int(cart[str(lant.id)])
            for lant in lanturi
        ]
