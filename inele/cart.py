...
class Cart:
    ...
    def add(self, inel_id, quantity):
        inel_id_str = str(inel_id)

        if inel_id_str in self._data:  # { '4': '10' }
            old_quantity = self._data[inel_id_str]
            old_quantity = int(old_quantity)

            new_quantity = old_quantity + quantity

            self._data[inel_id_str] = str(new_quantity)
        else:
            self._data[inel_id_str] = str(quantity)

        self._save()

    def remove(self, inel_id):
        del self._data[str(inel_id)]
        self._save()
