...
@register.inclusion_tag(filename='pandative/tags/cart.html', name='cart_link')
def get_cart_link(session):
    cart = session.get('cart', {})
    pandativ_ids = cart.keys()

    print('cart', cart)
    pandative = Produse.objects.filter(id__in=pandativ_ids)
    print('cart.values()', list(cart.values()))

    total = sum(
        [
            float(pandativ.price) * int(cart[str(pandativ.id)])
            for pandativ in pandative
        ]
    )  # sum([49.90 * 1, 24.90 * 1]) => sum([49.90, 24.90])

    return {
        'items': len(pandativ_ids),
        'total': '%.2f' % total
    }
